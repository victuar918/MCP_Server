/**
 * ASTERION AI Evolution Engine v5.23
 * v5.23: +phone_exec (Termux 쉘 실행 — 폰 서버 진단/배포/자기재시작)
 * v5.21: video_create_script & append_sheet_row Script(G) reject gate (close remaining Korean input paths); +video_write_safe (Doc-JSON write-by-reference, no LLM re-emit) +video_verify_script (server-side NFC compare sheet vs Doc)
...
const ALL_TOOLS = [
  ...
  {name:'get_timezone',description:'위도/경도+날짜로 타임존 반환. Google Timezone API 사용 — 역사적 DST 포함.',inputSchema:{type:'object',properties:{latitude:{type:'number'},longitude:{type:'number'},dateTime:{type:'string'}},required:['latitude','longitude','dateTime']}},
  ...
  {name:'video_append_script',description:'★ 기존 VS_ 시트에 스크립트 행 누적 추가(append). video_create_script로 시트 생성 후 섹션별 호출 — 전체 재작성 없이 마지막 데이터 행 다음에 추가(이전 섹션 보존, 토큰 절약).',inputSchema:{type:'object',properties:{sheet_name:{type:'string'},script_rows:{type:'array',items:{type:'object'}},spreadsheet_id:{type:'string'}},required:['sheet_name','script_rows']}},
  ...
  {name:'video_delete_script',description:'★ YouTube 업로드 완료 후 대본 시트 삭제. 안전장치: confirm_uploaded:true 가 없으면 거부합니다.',inputSchema:{type:'object',properties:{sheet_name:{type:'string'},confirm_uploaded:{type:'boolean'},spreadsheet_id:{type:'string'}},required:['sheet_name']}},
  {name:'video_write_safe',description:'Materialize a VS_ video script sheet from a Google Doc whose text is pure JSON {video_meta?,script_rows:[...],coin?,date?,sheet_name?}. Write-by-reference: the LLM does not re-emit Korean; the server reads the Doc and writes the sheet. mode:create(default) writes meta+header+rows from A1 and refuses if the sheet already has script rows; mode:append adds only data rows after the last row (for splitting a large video into per-section Docs, each materialized without LLM re-emission). sheet_name overrides the VS_{coin}_{date} name. dry_run:true validates Script(G) and returns per-row Script(G)/Card_Main character counts without writing (pre-write length gate). Rejects if Script(G) has latin/isolated-jamo/emoji.',inputSchema:{type:'object',properties:{source_doc_id:{type:'string'},coin:{type:'string'},date:{type:'string'},sheet_name:{type:'string'},mode:{type:'string',enum:['create','append']},dry_run:{type:'boolean'},spreadsheet_id:{type:'string'},script_json:{type:'string'},row_targets:{type:'array',items:{type:'number'},description:'Per-row minimum Script char targets, same order/length as script_rows. Checked BEFORE any write; shortfall rejects atomically (ROW_TARGET_SHORTFALL). Use 0 to skip a row.'}},required:[]}},
  {name:'video_verify_script',description:'Deterministically compare a VS_ sheet against its source JSON (script_json string, or a Google Doc via source_doc_id) field-by-field after NFC. Detects silent syllable substitution. Returns match boolean, per-field first-diff index and chars. No LLM re-emission.',inputSchema:{type:'object',properties:{sheet_name:{type:'string'},source_doc_id:{type:'string'},spreadsheet_id:{type:'string'},script_json:{type:'string'}},required:['sheet_name']}},
  ...
  {name:'phone_exec',description:'★ 폰 서버(Termux) 쉘 명령 실행. cwd 기본 ~/srv/MCP_Server, timeout_ms 기본 60000(최대 300000), detach:true는 백그라운드 실행 후 즉시 응답(자기 재시작용). 출력 8000자 절단.',inputSchema:{type:'object',properties:{command:{type:'string'},cwd:{type:'string'},timeout_ms:{type:'number'},detach:{type:'boolean'}},required:['command']}},
  ...
  {name:'call_gpt',description:'GPT AI 직접 호출.',inputSchema:{type:'object',properties:{prompt:{type:'string'},system_prompt:{type:'string'},model:{type:'string'},max_tokens:{type:'number'},previous_round_context:{type:'object'}},required:['prompt']}},
  ...
  // ★ v5.18: 시트 행 삭제/삽입
  {name:'delete_sheet_row',description:'★ Google Sheets 행 삭제. row_index(1-based) 또는 key_column+key_value로 행 특정.',inputSchema:{type:'object',properties:{spreadsheetId:{type:'string'},sheet_name:{type:'string'},row_index:{type:'number'},key_column:{type:'string'},key_value:{type:'string'}},required:['spreadsheetId','sheet_name']}},
  {name:'insert_sheet_row',description:'★ Google Sheets 특정 위치에 행 삽입. row_index 행 위에 삽입. values 지정 시 데이터 함께 입력.',inputSchema:{type:'object',properties:{spreadsheetId:{type:'string'},sheet_name:{type:'string'},row_index:{type:'number'},values:{type:'array',items:{type:'string'}}},required:['spreadsheetId','sheet_name','row_index']}},
  // ★ v5.12: 부분 업데이트 툴
...
const L3=new Set(['github_read_file','github_write_file','github_list_files','gh_push_files','github_patch_file','sheets_read','sheets_write','sheets_update_row','delete_sheet_row','insert_sheet_row','http_request','get_system_status','append_sheet_row','phone_exec']);
...
async function execBTR(n,a){
  ...
  if(!tok&&!['btr_init_candidate_slots','btr_consensus_analyzer','btr_conflict_axis_finder','validate_sclass_gate','video_init_sheets','video_create_script','video_read_script','video_update_row_status','video_delete_script','video_write_safe','video_verify_script','video_append_script'].includes(n))return{error:'Google 인증 실패'};
  ...
  if(n==='video_create_script'){
    ...
    if(ssId===ARCHIVE_SS_ID)return{error:'REFUSED_ARCHIVE_TARGET',message:'영상 스크립트 시트는 Archive SS에 생성할 수 없습니다. spreadsheet_id를 생략하거나 영상 전용 SS를 지정하세요.',attempted_spreadsheet_id:ssId};
    const today=new Date().toISOString().slice(0,10).replace(/-/g,'');
    const sheetName=`VS_${a.coin}_${a.date||today}`;const _sbad=[];(a.script_rows||[]).forEach((r,i)=>{const _sv=sanitizeVideoText(r.Script||'');const _iss=scriptCellIssues(_sv);if(_iss.length)_sbad.push({row:i,field:'Script',value:_sv.slice(0,40),issues:_iss});});if(_sbad.length)return{error:'script_validation_failed',rejected:true,cells:_sbad};const _cxg=await fetchWithTimeout(`https://sheets.googleapis.com/v4/spreadsheets/${ssId}/values/${encodeURIComponent(sheetName)}`,{headers:{Authorization:`Bearer ${gTok}`}},10000).catch(()=>null);if(_cxg&&_cxg.ok){const _cxv=((await _cxg.json()).values)||[];let _chi=-1;for(let i=0;i<_cxv.length;i++){if((_cxv[i]||[])[0]==='Section'){_chi=i;break;}}const _cExisting=_chi>=0?_cxv.slice(_chi+1).filter(r=>(r||[]).some(c=>String(c||'').trim())).length:0;if(_cExisting>0)return{error:'REFUSED_OVERWRITE_EXISTING',message:`시트 '${sheetName}'에 이미 스크립트 ${_cExisting}행이 있어 덮어쓰기를 거부합니다. video_append_script를 사용하세요.`,sheet_name:sheetName,existing_rows:_cExisting};}
  ...
  if(n==='video_append_script'){
    const gTok=await getGCPToken();if(!gTok)return{error:'GCP ADC 인증 실패'};
    const ssId=a.spreadsheet_id||VIDEO_SS_ID;
    if(ssId===ARCHIVE_SS_ID)return{error:'REFUSED_ARCHIVE_TARGET',message:'영상 스크립트 시트는 Archive SS에 추가할 수 없습니다.',attempted_spreadsheet_id:ssId};
    if(!a.sheet_name)return{error:'sheet_name required'};
    const _arows=Array.isArray(a.script_rows)?a.script_rows:[];
    if(!_arows.length)return{error:'script_rows 비어있음 (추가할 행이 없습니다)'};
    const _abad=[];_arows.forEach((r,i)=>{const _sv=sanitizeVideoText(r.Script||'');const _iss=scriptCellIssues(_sv);if(_iss.length)_abad.push({row:i,field:'Script',value:_sv.slice(0,40),issues:_iss});});if(_abad.length)return{error:'script_validation_failed',rejected:true,cells:_abad};
    const _agr=await fetchWithTimeout(`https://sheets.googleapis.com/v4/spreadsheets/${ssId}/values/${encodeURIComponent(a.sheet_name)}`,{headers:{Authorization:`Bearer ${gTok}`}},10000);
    if(!_agr.ok)return{error:`시트 읽기 실패 ${_agr.status} — video_create_script로 시트를 먼저 생성하세요`,sheet_name:a.sheet_name};
    const _acur=((await _agr.json()).values)||[];
    const _anext=_acur.length+1;
    const _amap=_arows.map(r=>[r.Section||'',String(r.Speaker||'1'),sanitizeVideoText(r.Card_Main||''),sanitizeVideoText(r.Card_Sub||''),sanitizeVideoText(r.Card_Desc||''),sanitizeVideoText(r.Highlight_Word||''),sanitizeVideoText(r.Script||''),r.BG_File||'',r.Animation||'A',r.Card_Style||'DEFAULT','READY',sanitizeVideoText(r.Note||''),r.BG_Effect||'NONE',r.BG_Transition||'FADE',r.Card_ExtraEffect||'NONE',r.Lottie_File||'NONE',r.Sticker_File||'NONE',r.Gradient_Preset||'DEFAULT']);
    const _awr=await fetchWithTimeout(`https://sheets.googleapis.com/v4/spreadsheets/${ssId}/values/${encodeURIComponent(a.sheet_name+'!A'+_anext)}?valueInputOption=USER_ENTERED`,{method:'PUT',headers:{Authorization:`Bearer ${gTok}`,'Content-Type':'application/json'},body:JSON.stringify({values:_amap})},15000);
    if(!_awr.ok)return{error:`append 실패 ${_awr.status}`};
    return{success:true,sheet_name:a.sheet_name,appended_rows:_amap.length,start_row:_anext,end_row:_anext+_amap.length-1};
  }
  ...
  if(n==='video_delete_script'){
    const gTok=await getGCPToken();if(!gTok)return{error:'GCP ADC 인증 실패'};
    if(a.confirm_uploaded!==true)return{error:'REFUSED_DELETE_NEEDS_CONFIRM',message:`시트 삭제는 confirm_uploaded:true 가 있어야 합니다. YouTube 업로드 완료가 확인된 경우에만 호출하세요.`,sheet_name:a.sheet_name};
    const ssId=a.spreadsheet_id||VIDEO_SS_ID;
  ...
  if(n==='video_write_safe'){const gTok=await getGCPToken();if(!gTok)return{error:'GCP ADC required for sheet write'};let _spec=null;if(a.script_json!==undefined&&a.script_json!==null&&String(a.script_json).trim()!==''){const _sj=typeof a.script_json==='string'?a.script_json:JSON.stringify(a.script_json);try{_spec=JSON.parse(_sj);}catch(e){return{error:'script_json parse failed: '+e.message,hint:'script_json must be pure JSON {video_meta?,script_rows:[...]} — standard quotes only, \\n as two chars inside strings'};}}if(!_spec){const oTok=await getGoogleToken();if(!oTok)return{error:'OAuth token required for Doc read'};if(!a.source_doc_id)return{error:'source_doc_id or script_json required'};const _dr=await fetchWithTimeout(`https://docs.googleapis.com/v1/documents/${a.source_doc_id}`,{headers:{Authorization:`Bearer ${oTok}`}},15000);if(!_dr.ok)return{error:`Doc read failed ${_dr.status}`};const _dd=await _dr.json();const _txt=(_dd.body?.content||[]).flatMap(b=>b.paragraph?.elements??[]).map(el=>el.textRun?.content??'').join('');try{_spec=JSON.parse(_txt);}catch(e){return{error:'Doc JSON parse failed: '+e.message+' (Doc text must be pure JSON {video_meta?,script_rows:[...],coin?,date?})'};}}const _rows=_spec.script_rows||[];const _vm=_spec.video_meta||{};const _coin=a.coin||_spec.coin||'UNKNOWN';const _ssId=a.spreadsheet_id||VIDEO_SS_ID;if(_ssId===ARCHIVE_SS_ID)return{error:'REFUSED_ARCHIVE_TARGET',message:'영상 스크립트 시트는 Archive SS에 생성할 수 없습니다.',attempted_spreadsheet_id:_ssId};const _today=new Date().toISOString().slice(0,10).replace(/-/g,'');const _sheet=a.sheet_name||_spec.sheet_name||`VS_${_coin}_${a.date||_spec.date||_today}`;const _exg=await fetchWithTimeout(`https://sheets.googleapis.com/v4/spreadsheets/${_ssId}/values/${encodeURIComponent(_sheet)}`,{headers:{Authorization:`Bearer ${gTok}`}},10000).catch(()=>null);if(_exg&&_exg.ok){const _exv=((await _exg.json()).values)||[];let _hi=-1;for(let i=0;i<_exv.length;i++){if((_exv[i]||[])[0]==='Section'){_hi=i;break;}}const _existingData=_hi>=0?_exv.slice(_hi+1).filter(r=>(r||[]).some(c=>String(c||'').trim())).length:0;const _isAppend=a.mode==='append';if(_existingData>0&&!_isAppend)return{error:'REFUSED_OVERWRITE_EXISTING',message:`시트 '${_sheet}'에 이미 스크립트 ${_existingData}행이 있어 덮어쓰기를 거부합니다. 새 시트명을 쓰거나 mode:append를 사용하세요.`,sheet_name:_sheet,existing_rows:_existingData};if(_isAppend&&_hi<0)return{error:'APPEND_BEFORE_CREATE',message:`시트 '${_sheet}'에 헤더가 없습니다. 먼저 mode:create로 시트를 생성하세요.`,sheet_name:_sheet};}else if(a.mode==='append')return{error:'APPEND_BEFORE_CREATE',message:`시트 '${_sheet}'을 읽을 수 없어 append 할 수 없습니다. 먼저 mode:create로 생성하세요.`,sheet_name:_sheet};const _sbad=[];_rows.forEach((r,i)=>{const _sv=sanitizeVideoText(r.Script||'');const _iss=scriptCellIssues(_sv);if(_iss.length)_sbad.push({row:i,field:'Script',value:_sv.slice(0,40),issues:_iss});});if(_sbad.length)return{error:'script_validation_failed',rejected:true,cells:_sbad};const _CANON=['Section','Speaker','Card_Main','Card_Sub','Card_Desc','Highlight_Word','Script','BG_File','Animation','Card_Style','Status','Note','BG_Effect','BG_Transition','Card_ExtraEffect','Lottie_File','Sticker_File','Gradient_Preset'];const _badKeys=[];_rows.forEach((r,i)=>{const _uk=Object.keys(r||{}).filter(k=>!_CANON.includes(k));if(_uk.length)_badKeys.push({row:i,unknown_keys:_uk});});if(_badKeys.length)return{error:'UNKNOWN_FIELD_KEYS',rejected:true,message:'script_rows JSON keys must exactly match the sheet header field names (case-sensitive). Do not use column letters (A,B,C..R) or other names.',canonical_keys:_CANON,bad_rows:_badKeys};const _emptySec=[];_rows.forEach((r,i)=>{if(!String((r||{}).Section||'').trim())_emptySec.push(i);});if(_emptySec.length)return{error:'EMPTY_SECTION',rejected:true,message:'Rows with empty Section detected (key mismatch or missing Section). Every row needs a Section field.',canonical_keys:_CANON,empty_section_rows:_emptySec};if(a.dry_run===true){const _dryRows=_rows.map((r,i)=>({index:i,section:r.Section||'',speaker:String(r.Speaker||'1'),script_chars:sanitizeVideoText(r.Script||'').length,card_main_chars:sanitizeVideoText(r.Card_Main||'').length}));return{dry_run:true,sheet_name:_sheet,mode:a.mode==='append'?'append':'create',row_count:_rows.length,total_script_chars:_dryRows.reduce((s,x)=>s+x.script_chars,0),rows:_dryRows,zero_script_rows:_dryRows.filter(x=>x.script_chars===0).map(x=>x.index),script_validation:'pass',note:'dry_run: counted chars + key/section validated, no sheet write'};}if(Array.isArray(a.row_targets)&&a.row_targets.length>0){if(a.row_targets.length!==_rows.length)return{error:'ROW_TARGETS_LENGTH_MISMATCH',rejected:true,message:`row_targets length ${a.row_targets.length} does not match script_rows length ${_rows.length}. Provide one target per row, in order.`,row_targets_length:a.row_targets.length,script_rows_length:_rows.length};const _allCounts=_rows.map((r,i)=>({index:i,section:r.Section||'',script_chars:sanitizeVideoText(r.Script||'').length,target:Number(a.row_targets[i])||0}));const _short=_allCounts.filter(x=>x.target>0&&x.script_chars<x.target).map(x=>({...x,shortfall:x.target-x.script_chars}));if(_short.length>0)return{error:'ROW_TARGET_SHORTFALL',rejected:true,message:'Some rows are shorter than their row_target. Nothing was written. Extend the listed rows and resend.',short_rows:_short,rows:_allCounts,total_script_chars:_allCounts.reduce((s,x)=>s+x.script_chars,0)};}const _cr=await fetchWithTimeout(`https://sheets.googleapis.com/v4/spreadsheets/${_ssId}:batchUpdate`,{method:'POST',headers:{Authorization:`Bearer ${gTok}`,'Content-Type':'application/json'},body:JSON.stringify({requests:[{addSheet:{properties:{title:_sheet}}}]})},15000);if(!_cr.ok){const _et=await _cr.text();if(!_et.includes('already exists')&&!_et.includes('A sheet with the name'))return{error:`sheet create failed ${_cr.status}`};}const _meta=[['[VIDEO_META]','YouTube_Title',_vm.youtube_title||''],['','Top_Watermark',_vm.top_watermark||''],['','Thumbnail_Text',_vm.thumbnail_text||''],['','Main_BGM',_vm.main_bgm||''],[''],['Section','Speaker','Card_Main','Card_Sub','Card_Desc','Highlight_Word','Script','BG_File','Animation','Card_Style','Status','Note','BG_Effect','BG_Transition','Card_ExtraEffect','Lottie_File','Sticker_File','Gradient_Preset'],..._rows.map(r=>[r.Section||'',String(r.Speaker||'1'),sanitizeVideoText(r.Card_Main||''),sanitizeVideoText(r.Card_Sub||''),sanitizeVideoText(r.Card_Desc||''),sanitizeVideoText(r.Highlight_Word||''),sanitizeVideoText(r.Script||''),r.BG_File||'',r.Animation||'A',r.Card_Style||'DEFAULT','READY',sanitizeVideoText(r.Note||''),r.BG_Effect||'NONE',r.BG_Transition||'FADE',r.Card_ExtraEffect||'NONE',r.Lottie_File||'NONE',r.Sticker_File||'NONE',r.Gradient_Preset||'DEFAULT'])];const _dataRows=_meta.slice(6);if(a.mode==='append'){const _agr2=await fetchWithTimeout(`https://sheets.googleapis.com/v4/spreadsheets/${_ssId}/values/${encodeURIComponent(_sheet)}`,{headers:{Authorization:`Bearer ${gTok}`}},10000);if(!_agr2.ok)return{error:`append 시트 재읽기 실패 ${_agr2.status}`,sheet_name:_sheet};const _acur2=((await _agr2.json()).values)||[];const _anext2=_acur2.length+1;const _awr=await fetchWithTimeout(`https://sheets.googleapis.com/v4/spreadsheets/${_ssId}/values/${encodeURIComponent(_sheet+'!A'+_anext2)}?valueInputOption=USER_ENTERED`,{method:'PUT',headers:{Authorization:`Bearer ${gTok}`,'Content-Type':'application/json'},body:JSON.stringify({values:_dataRows})},15000);return{success:_awr.ok,sheet_name:_sheet,mode:'append',appended_rows:_dataRows.length,start_row:_anext2,end_row:_anext2+_dataRows.length-1,source_doc_id:a.source_doc_id,note:'appended from Doc without LLM re-emission',url:`https://docs.google.com/spreadsheets/d/${_ssId}`};}const _wr=await fetchWithTimeout(`https://sheets.googleapis.com/v4/spreadsheets/${_ssId}/values/${encodeURIComponent(_sheet+'!A1')}?valueInputOption=USER_ENTERED`,{method:'PUT',headers:{Authorization:`Bearer ${gTok}`,'Content-Type':'application/json'},body:JSON.stringify({values:_meta})},15000);return{success:_wr.ok,sheet_name:_sheet,mode:'create',total_rows:_meta.length,script_rows:_rows.length,source_doc_id:a.source_doc_id,note:'written from Doc without LLM re-emission',url:`https://docs.google.com/spreadsheets/d/${_ssId}`};}

  if(n==='video_verify_script'){const gTok=await getGCPToken();if(!gTok)return{error:'GCP ADC required for sheet read'};if(!a.sheet_name)return{error:'sheet_name required'};let _vspec=null;if(a.script_json!==undefined&&a.script_json!==null&&String(a.script_json).trim()!==''){const _vsj=typeof a.script_json==='string'?a.script_json:JSON.stringify(a.script_json);try{_vspec=JSON.parse(_vsj);}catch(e){return{error:'script_json parse failed: '+e.message};}}let oTok=null;if(!_vspec){oTok=await getGoogleToken();if(!oTok)return{error:'OAuth token required for Doc read'};if(!a.source_doc_id)return{error:'source_doc_id or script_json required'};}const _ssId=a.spreadsheet_id||VIDEO_SS_ID;const _sr=await fetchWithTimeout(`https://sheets.googleapis.com/v4/spreadsheets/${_ssId}/values/${encodeURIComponent(a.sheet_name)}`,{headers:{Authorization:`Bearer ${gTok}`}},10000);if(!_sr.ok)return{error:`sheet read failed ${_sr.status}`};const _srows=((await _sr.json()).values)||[];let _ss=-1;for(let i=0;i<_srows.length;i++){if(_srows[i][0]==='Section'){_ss=i+1;break;}}const _sheetRows=_ss>=0?_srows.slice(_ss).filter(r=>r[0]):[];let _spec;if(_vspec){_spec=_vspec;}else{const _dr=await fetchWithTimeout(`https://docs.googleapis.com/v1/documents/${a.source_doc_id}`,{headers:{Authorization:`Bearer ${oTok}`}},15000);if(!_dr.ok)return{error:`Doc read failed ${_dr.status}`};const _dd=await _dr.json();const _txt=(_dd.body?.content||[]).flatMap(b=>b.paragraph?.elements??[]).map(el=>el.textRun?.content??'').join('');try{_spec=JSON.parse(_txt);}catch(e){return{error:'Doc JSON parse failed: '+e.message};}}const _docRows=_spec.script_rows||[];const _FIELDS=[['Card_Main',2],['Card_Sub',3],['Card_Desc',4],['Highlight_Word',5],['Script',6],['Note',11]];const _norm=v=>(typeof v==='string'?v:String(v==null?'':v)).normalize('NFC');const _n=Math.max(_sheetRows.length,_docRows.length);const _diffs=[];for(let i=0;i<_n;i++){const _row=_sheetRows[i]||[];const _drow=_docRows[i]||{};_FIELDS.forEach(([f,ci])=>{const _sv=_norm(_row[ci]||'');const _dv=_norm(_drow[f]||'');if(_sv!==_dv){let _di=0;while(_di<_sv.length&&_di<_dv.length&&_sv[_di]===_dv[_di])_di++;_diffs.push({row:i,field:f,first_diff_index:_di,sheet_char:_sv[_di]||null,doc_char:_dv[_di]||null,sheet:_sv.slice(0,40),doc:_dv.slice(0,40)});}});}const _countOk=_sheetRows.length===_docRows.length;return{match:_diffs.length===0&&_countOk&&_sheetRows.length>0,row_count_match:_countOk,total_rows_compared:_n,sheet_rows:_sheetRows.length,doc_rows:_docRows.length,diffs:_diffs,...(_sheetRows.length===0?{warning:'sheet has 0 parsed rows (empty Section column?) - likely write failed or key mismatch'}:{})};}
...
async function execSystem(n,a){
  ...
  if(['sheets_read','sheets_write','append_sheet_row'].includes(n)){
    ...
    if(n==='append_sheet_row'){const _avid=isVideoSheet(a.range);const _arow=_avid?sanitizeRows([a.values])[0]:a.values;if(_avid){const _abad=validateScriptRows([_arow],a.range);if(_abad)return{error:'script_validation_failed',rejected:true,cells:_abad};}const r=await fetchWithTimeout(`https://sheets.googleapis.com/v4/spreadsheets/${a.spreadsheetId}/values/${encodeURIComponent(a.range)}:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`,{method:'POST',headers:{Authorization:`Bearer ${tok}`,'Content-Type':'application/json'},body:JSON.stringify({values:[_arow]})});return r.ok?await r.json():{error:`Sheets ${r.status}`};}
  }
  ...
  if(n==='delete_sheet_row'){const tok=await getGoogleToken();if(!tok)return{error:'Google 인증 실패'};const{spreadsheetId,sheet_name,row_index,key_column,key_value}=a;const si=await fetchWithTimeout(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}?fields=sheets.properties`,{headers:{Authorization:`Bearer ${tok}`}},10000);if(!si.ok)return{error:`SS 조회 실패 ${si.status}`};const sh=(await si.json()).sheets?.find(s=>s.properties.title===sheet_name);if(!sh)return{error:`시트 없음: ${sheet_name}`};const sheetId=sh.properties.sheetId;let targetRow=row_index;if(!targetRow&&key_column&&key_value){const dr=await fetchWithTimeout(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${encodeURIComponent(sheet_name)}`,{headers:{Authorization:`Bearer ${tok}`}},10000);if(!dr.ok)return{error:`읽기 실패 ${dr.status}`};const rows=((await dr.json()).values)||[],hdr=rows[0]||[],ki=hdr.indexOf(key_column);if(ki<0)return{error:`컬럼 없음: ${key_column}`};const ri=rows.findIndex((r,i)=>i>0&&String(r[ki]||'').trim()===String(key_value).trim());if(ri<0)return{error:`키 값 없음: ${key_value}`};targetRow=ri+1;}if(!targetRow)return{error:'row_index 또는 key_column+key_value 필요'};const delR=await fetchWithTimeout(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}:batchUpdate`,{method:'POST',headers:{Authorization:`Bearer ${tok}`,'Content-Type':'application/json'},body:JSON.stringify({requests:[{deleteDimension:{range:{sheetId,dimension:'ROWS',startIndex:targetRow-1,endIndex:targetRow}}}]})},15000);if(!delR.ok)return{error:`행 삭제 실패 ${delR.status}: ${(await delR.text()).slice(0,200)}`};return{success:true,deleted_row:targetRow,sheet_name};}
  if(n==='insert_sheet_row'){const tok=await getGoogleToken();if(!tok)return{error:'Google 인증 실패'};const{spreadsheetId,sheet_name,row_index,values}=a;const si=await fetchWithTimeout(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}?fields=sheets.properties`,{headers:{Authorization:`Bearer ${tok}`}},10000);if(!si.ok)return{error:`SS 조회 실패 ${si.status}`};const sh=(await si.json()).sheets?.find(s=>s.properties.title===sheet_name);if(!sh)return{error:`시트 없음: ${sheet_name}`};const sheetId=sh.properties.sheetId;const insR=await fetchWithTimeout(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}:batchUpdate`,{method:'POST',headers:{Authorization:`Bearer ${tok}`,'Content-Type':'application/json'},body:JSON.stringify({requests:[{insertDimension:{range:{sheetId,dimension:'ROWS',startIndex:row_index-1,endIndex:row_index},inheritFromBefore:false}}]})},15000);if(!insR.ok)return{error:`행 삽입 실패 ${insR.status}: ${(await insR.text()).slice(0,200)}`};if(values?.length){const wr=await fetchWithTimeout(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${encodeURIComponent(sheet_name+'!A'+row_index)}?valueInputOption=USER_ENTERED`,{method:'PUT',headers:{Authorization:`Bearer ${tok}`,'Content-Type':'application/json'},body:JSON.stringify({values:[values]})},15000);if(!wr.ok)return{error:`값 입력 실패 ${wr.status}`};}return{success:true,inserted_row:row_index,sheet_name,values_written:values?.length||0};}
  if(n==='phone_exec'){
    const cmd=a.command;
    if(!cmd||typeof cmd!=='string')return{error:'command 문자열 필요'};
    const HOME=process.env.HOME||process.cwd();
    let cwd=a.cwd?String(a.cwd):HOME+'/srv/MCP_Server';
    if(cwd.startsWith('~'))cwd=HOME+cwd.slice(1);
    if(!existsSync(cwd))return{error:'cwd 폴더가 없습니다: '+cwd};
    const sh=process.env.SHELL||(process.env.PREFIX?process.env.PREFIX+'/bin/bash':'/bin/sh');
    const MAX=8000,clip=s=>{s=String(s||'');return s.length>MAX?s.slice(0,MAX)+' ...[truncated '+(s.length-MAX)+' chars]':s;};
    if(a.detach===true){
      try{const ch=spawn(sh,['-c',cmd],{cwd,detached:true,stdio:'ignore'});ch.unref();return{detached:true,pid:ch.pid,shell:sh,cwd,command:cmd,note:'백그라운드 실행 — 출력 없음'};}
      catch(e){return{error:'spawn 실패: '+e.message,shell:sh,cwd};}
    }
    let t=Number(a.timeout_ms);if(!Number.isFinite(t)||t<=0)t=60000;t=Math.min(Math.max(t,1000),300000);
    return await new Promise(rs=>{exec(cmd,{cwd,shell:sh,timeout:t,maxBuffer:10485760,killSignal:'SIGKILL',encoding:'utf8'},(err,so,se)=>{const o={command:cmd,cwd,shell:sh,timeout_ms:t,exit_code:err?(typeof err.code==='number'?err.code:null):0,timed_out:!!(err&&err.killed),stdout:clip(so),stderr:clip(se)};if(err&&typeof err.code!=='number')o.error=err.message;rs(o);});});
  }
...
async function execWorkspace(n,a){
  ...
  if(n==='list_script_projects'){const r=await fetchWithTimeout(`https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent("mimeType='application/vnd.google-apps.script' and trashed=false")}&fields=files(id,name,modifiedTime,webViewLink)&pageSize=${a.max_results||20}`,{headers:{Authorization:`Bearer ${tok}`}});return r.ok?await r.json():{error:`Drive ${r.status}: ${await r.text()}`};}
  ...
  if(n==='delete_run_revision'){const gt=await getGCPToken();if(!gt)return{error:'GCP ADC 실패'};const p=a.project||GCP_PROJECT,rg=a.region||GCP_REGION;const svc=a.service_name||(a.revision_name||'').split('-').slice(0,-2).join('-');const r=await fetchWithTimeout(`https://run.googleapis.com/v2/projects/${p}/locations/${rg}/services/${svc}/revisions/${a.revision_name}`,{method:'DELETE',headers:{Authorization:`Bearer ${gt}`}});return{success:r.ok,status:r.status};}
...
async function execAI(n,a){
  ...
  if(n==='call_gemini'){const key=process.env.NVIDIA_API_KEY;if(!key)return{error:'GEMINI_API_KEY 미설정'};const model=a.model||'nvidia/nemotron-3-ultra-550b-a55b';const defaultSys=`<role>ASTERION BTR rubric analyst</role>\n<rubric total="100"><criterion id="event_fit" max="40"/><criterion id="navamsa_d9" max="20"/><criterion id="appearance" max="15"/><criterion id="sandhi" max="15"/><criterion id="consistency" max="10"/></rubric>\n<hard_stop>all_scores≥97 AND critical_issues=[] → S-Class CONFIRMED</hard_stop>\n<anti_anchoring_rule>Evaluate INDEPENDENTLY.</anti_anchoring_rule>\n<output_format>{"candidate_time":"HH:MM","analysis":"string","scores":{"event_fit":0,"navamsa_d9":0,"appearance":0,"sandhi":0,"consistency":0},"total":0,"critical_issues":[],"suggestions":[],"confidence":"LOW|MEDIUM|HIGH"}</output_format>\n<lang>Respond in Korean</lang>`;const sys=(a.system_prompt||defaultSys)+antiAnchoredCtx;const r=await fetchWithTimeout('https://integrate.api.nvidia.com/v1/chat/completions',{method:'POST',headers:{Authorization:'Bearer '+key,'Content-Type':'application/json'},body:JSON.stringify({model,max_tokens:a.max_tokens||8192,temperature:0.6,top_p:0.95,reasoning_budget:4096,chat_template_kwargs:{enable_thinking:true},messages:[{role:'system',content:sys},{role:'user',content:a.prompt}]})},180000);if(!r.ok)throw new Error('Nemotron '+r.status+': '+(await r.text()).slice(0,200));const d=await r.json();return{text:d.choices?.[0]?.message?.content||'',model};}
  if(n==='call_claude'){const key=process.env.ANTHROPIC_API_KEY;if(!key)return{error:'ANTHROPIC_API_KEY 미설정'};const model=a.model||'claude-sonnet-4-6';const defaultSys=`<role>ASTERION BTR rubric analyst</role>\n<anti_anchoring_rule>Evaluate INDEPENDENTLY.</anti_anchoring_rule>\n<output_format>{"candidate_time":"HH:MM","analysis":"string","scores":{"event_fit":0,"navamsa_d9":0,"appearance":0,"sandhi":0,"consistency":0},"total":0,"critical_issues":[],"suggestions":[]}</output_format>\n<lang>Respond in Korean</lang>`;const sys=(a.system_prompt||defaultSys)+antiAnchoredCtx;const r=await fetchWithTimeout('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'x-api-key':key,'anthropic-version':'2023-06-01','content-type':'application/json'},body:JSON.stringify({model,max_tokens:Math.max(a.max_tokens||8000,8000),system:sys,thinking:{type:'enabled',budget_tokens:5000},messages:[{role:'user',content:a.prompt}]})},180000);if(!r.ok)throw new Error(`Claude ${r.status}: ${(await r.text()).slice(0,200)}`);const d=await r.json();return{text:d.content?.find(b=>b.type==='text')?.text||'',model};}
  if(n==='call_gpt'){const key=process.env.DEEPSEEK_API_KEY;if(!key)return{error:'DEEPSEEK_API_KEY 미설정'};const model=a.model||'deepseek-v4-pro';const r=await fetchWithTimeout('https://api.deepseek.com/chat/completions',{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({model,max_tokens:a.max_tokens||8000,thinking:{type:'enabled'},reasoning_effort:'high',stream:false,messages:[{role:'system',content:a.system_prompt||'베다 점성술 BTR 전문가.'},{role:'user',content:a.prompt+antiAnchoredCtx}]})},180000);if(!r.ok)throw new Error('DeepSeek '+r.status+': '+(await r.text()).slice(0,200));const d=await r.json();return{text:d.choices?.[0]?.message?.content||'',model};}
  return{error:`미구현: ${n}`};
}
...
app.get('/sse',requireMcpAuth,(req,res)=>{
  ...
  sessions.set(sid,res);req.on('close',()=>sessions.delete(sid));//\n\n');}catch(_){clearInterval(ka);}},25000);req.on('close',()=>{clearInterval(ka);sessions.delete(sid);});
});
...
app.get('/',(_req,res)=>res.json({
  status:'running',server:'ASTERION AI Evolution Engine v5.23',
