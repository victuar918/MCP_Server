/**
 * ASTERION AI Evolution Engine v5.21
...
const L3=new Set(['github_read_file','github_write_file','github_list_files','gh_push_files','github_patch_file','sheets_read','sheets_write','sheets_update_row','delete_sheet_row','insert_sheet_row','http_request','get_system_status','append_sheet_row']);
...
async function execAI(n,a){
  ...
  if(n==='call_gpt'){const key=process.env.NVIDIA_API_KEY;if(!key)return{error:'OPENAI_API_KEY 미설정'};const model=a.model||'z-ai/glm-5.2';const r=await fetchWithTimeout('https://integrate.api.nvidia.com/v1/chat/completions',{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({model,max_tokens:a.max_tokens||8000,temperature:0.6,messages:[{role:'system',content:a.system_prompt||'베다 점성술 BTR 전문가.'},{role:'user',content:a.prompt+antiAnchoredCtx}]})},180000);if(!r.ok)throw new Error('GLM '+r.status+': '+(await r.text()).slice(0,200));const d=await r.json();return{text:d.choices?.[0]?.message?.content||'',model};}
  return{error:`미구현: ${n}`};
}
...
app.get('/',(_req,res)=>res.json({
  status:'running',server:'ASTERION AI Evolution Engine v5.22',
...
app.get('/diag/nvidia-tools',async(req,res)=>{try{const key=process.env.NVIDIA_API_KEY;if(!key)return res.json({ok:false,error:'NVIDIA_API_KEY missing'});const model=req.query.model||'z-ai/glm-5.2';const think=req.query.think==='on';const tc=req.query.tc||'auto';const body={model,messages:[{role:'user',content:'What is the weather in Seoul? Please use the get_weather tool.'}],tools:[{type:'function',function:{name:'get_weather',description:'Get current weather for a city',parameters:{type:'object',properties:{city:{type:'string'}},required:['city']}}}],tool_choice:tc,max_tokens:512,temperature:1,top_p:0.95};if(model.indexOf('nemotron')>=0)body.chat_template_kwargs={enable_thinking:think,force_nonempty_content:true};const r=await fetchWithTimeout('https://integrate.api.nvidia.com/v1/chat/completions',{method:'POST',headers:{Authorization:'Bearer '+key,'Content-Type':'application/json'},body:JSON.stringify(body)},25000);const txt=await r.text();if(!r.ok)return res.json({ok:false,status:r.status,body:txt.slice(0,800)});let d;try{d=JSON.parse(txt);}catch(e){return res.json({ok:false,parse_error:e.message,body:txt.slice(0,800)});}const m=d.choices?.[0]?.message;return res.json({ok:true,model,think,tool_choice:tc,finish_reason:d.choices?.[0]?.finish_reason,has_tool_calls:!!(m?.tool_calls?.length),tool_calls:m?.tool_calls||null,content:(m?.content||'').slice(0,400)});}catch(e){return res.json({ok:false,error:e.message});}});
