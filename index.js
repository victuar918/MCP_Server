...
async function execVedAstro(n,a){
  try{
    ...
    if(n==='get_timezone'){const _gtzTok=await getGoogleToken();if(!_gtzTok)return{error:'GCP ADC 인증 실패'};const _gtzTs=Math.floor(new Date(dt||new Date().toISOString()).getTime()/1000);const r=await fetchWithTimeout(`https://maps.googleapis.com/maps/api/timezone/json?location=${la},${lo}&timestamp=${_gtzTs}`,{headers:{Authorization:`Bearer ${_gtzTok}`}},15000);if(!r.ok)return{error:`Google Timezone API ${r.status}`};const j=await r.json();if(j.status!=='OK')return{error:`Google Timezone: ${j.status}${j.errorMessage?' - '+j.errorMessage:''}`};const _gtzTotal=j.rawOffset+j.dstOffset;const _gtzSign=_gtzTotal>=0?'+':'-';const _gtzAbs=Math.abs(_gtzTotal);return{timezone_id:j.timeZoneId,timezone_name:j.timeZoneName,raw_offset_sec:j.rawOffset,dst_offset_sec:j.dstOffset,utc_offset:`${_gtzSign}${String(Math.floor(_gtzAbs/3600)).padStart(2,'0')}:${String(Math.floor(_gtzAbs%3600/60)).padStart(2,'0')}`};}
