...
app.get('/sse',requireMcpAuth,(req,res)=>{
  ...
  sessions.set(sid,res);res.on('error',()=>{});const ka=setInterval(()=>{if(res.destroyed||res.writableEnded){clearInterval(ka);return;}try{res.write(': ka\n\n');}catch(_){clearInterval(ka);}},25000);req.on('close',()=>{clearInterval(ka);sessions.delete(sid);});
});
app.post('/message',requireMcpAuth,async(req,res)=>{
  const sseRes=sessions.get(req.query.sessionId);if(!sseRes||sseRes.destroyed||sseRes.writableEnded)return res.status(404).json({error:'세션 없음'});
...
process.on('unhandledRejection',(r)=>{console.error('[GUARD] unhandledRejection:',r&&r.message?r.message:String(r));});
process.on('uncaughtException',(e)=>{console.error('[GUARD] uncaughtException:',e&&e.message?e.message:String(e));});
