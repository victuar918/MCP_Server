...
const ALL_TOOLS = [
  ...
  {name:'video_write_safe',description:'Materialize a VS_ video script sheet from a Google Doc whose text is the script as pure JSON {video_meta?,script_rows:[...],coin?,date?}. Write-by-reference: the LLM does not re-emit Korean; the server reads the Doc and writes the sheet. Rejects if Script(G) has latin/isolated-jamo/emoji.',inputSchema:{type:'object',properties:{source_doc_id:{type:'string'},coin:{type:'string'},date:{type:'string'},spreadsheet_id:{type:'string'}},required:['source_doc_id']}},
