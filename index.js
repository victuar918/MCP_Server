...
async function execVedAstro(n,a){
  try{
    ...
    if(n==='get_timezone'){const{t:_tzt,d:_tzd}=dtToVed(dt);const r=await fetchWithTimeout(`${VEDASTRO_BASE}/Calculate/TimeZone${vedPath(la,lo,_tzt,_tzd,tz)}`);return r.ok?await r.json():{error:`HTTP ${r.status}`};}
