...
async function execVedAstro(n,a){
  try{
    ...
    if(n==='get_timezone'){const r=await fetchWithTimeout(`https://timeapi.io/api/TimeZone/coordinate?latitude=${la}&longitude=${lo}`,{},15000);if(!r.ok)return{error:`TimeZone API ${r.status}`};const j=await r.json();const _tzSec=j.currentUtcOffset?.seconds||0;const _tzSign=_tzSec>=0?'+':'-';const _tzAbs=Math.abs(_tzSec);return{timezone_id:j.timeZone,utc_offset:`${_tzSign}${String(Math.floor(_tzAbs/3600)).padStart(2,'0')}:${String(Math.floor(_tzAbs%3600/60)).padStart(2,'0')}`,has_dst:j.hasDayLightSaving,dst_active:j.isDayLightSavingActive};}
